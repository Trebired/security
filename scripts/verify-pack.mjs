...
const log = resolveLogger({ source: "@trebired/security" });
...
const corsDir = path.join(rootDir, "node_modules", "cors");
const corsTypesDir = path.join(rootDir, "node_modules", "@types", "cors");
...
async function writeConsumerManifest(consumerDir, tarballPath) {
  await fs.writeFile(path.join(consumerDir, "package.json"), JSON.stringify({
        name: "security-pack-smoke",
        private: true,
        type: "module",
        dependencies: {
          "@package/logger-adapter": `file:${loggerAdapterDir}`,
          "@package/security": `file:${tarballPath}`,
          "cors": `file:${corsDir}`,
        },
        devDependencies: {
          "@types/cors": `file:${corsTypesDir}`,
...
async function writeConsumerTypecheckFixture(consumerDir) {
  await fs.writeFile(path.join(consumerDir, "index.ts"), [
      'import { createCorsOptionsDelegate, defineConfig } from "@package/security";',
      "",
      "const config = defineConfig({ forVersion: \"0.1.0\", systems: { nonce: { order: 0 } } });",
      "const delegate = createCorsOptionsDelegate({ credentials: true });",
      "void config;",
      "void delegate;",
    ].join("\n"));
}
...
async function writeConsumerRuntimeFixture(consumerDir) {
  await fs.writeFile(path.join(consumerDir, "runtime.mjs"), [
      'import { attachSecurity } from "@package/security";',
      "",
      "const calls = [];",
      "const app = { use: (mw) => calls.push(typeof mw), set: () => {} };",
      "await attachSecurity(app, { config: { forVersion: \"0.1.0\" } });",
      "console.log(calls.length, calls.every((t) => t === \"function\"));",
    ].join("\n"));
}
